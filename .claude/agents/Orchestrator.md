...
### 팀 리더 규칙
- **delegate 모드** 사용 (Shift+Tab): 직접 코드 작성 금지
- 팀원 생성, 태스크 배분, 의존성 관리만 수행
- Monday.com/Slack 연동은 리드가 직접 처리

### 팀 관리
- 팀원의 plan approval 요청을 검토하고 승인/거절
- 팀원 간 파일 충돌 발생 시 중재
- 팀원이 idle 상태가 되면 다음 태스크 할당 또는 종료

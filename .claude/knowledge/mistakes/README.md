...
## M-007: Claude API max_tokens와 JSON 완성도 (2026-04-02~03)
- **증상**: Claude 응답 JSON이 잘려서 파싱 실패 → 로컬 폴백(저품질)
- **원인**: 37명 선수의 점수+summary를 JSON으로 요청 → 8192 토큰 초과. 32768으로 올리면 SDK가 스트리밍 필수 요구
- **해결**: Claude에게 텍스트만 요청(players 배열 제거), 서버에서 조립
- **교훈**: LLM에게 대량 구조화 데이터를 JSON으로 요청하지 말 것. 텍스트 생성만 맡기고 데이터는 서버에서 계산

## M-008: 한국어 상세 포지션 미매핑 (2026-04-03)
- **증상**: 포천 xlsx에서 "골키퍼"/"센터백" 등 한국어 포지션 → 서버에서 "MF"로 폴백
- **원인**: 서버 _normalize_pos에 한국어 상세 포지션 매핑 없었음
- **해결**: 30개+ 한국어 상세 포지션 → GK/DF/MF/FW 매핑 테이블 추가
- **교훈**: K3 구단 xlsx는 GK/DF/MF/FW가 아닌 "레프트백"/"수비형 미드필더" 등 상세 포지션 사용

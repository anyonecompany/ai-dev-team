#!/bin/bash
# Security Auto Trigger - PostToolUse Hook (Edit/Write)
...
SECURITY_PATTERNS = [
    ...
    r'credential', r'permission', r'middleware',
    r'\.env', r'config/security',
    r'rls', r'policy', r'migration',
    r'api/', r'route\.',
...
marker = os.path.join(marker_dir, f'aidevteam-{safe_path}')
...
if [[ -n "$RESULT" ]]; then
    ...
    echo "[Security] 보안 관련 파일 수정 감지: ${BASENAME} (패턴: ${PATTERN}). 커밋 전 보안 리뷰 실행을 권장합니다." >&2
fi
